  ...
  context "when representing" do
    ...
    context "to JSON" do
      it "represents properties" do
        ...
        expect(search_result.extend(subject).to_json).to be_json_eql(names).excluding(:items)
      end

      it "represents items" do
        json_representation = search_result.extend(subject).to_json
  ...
  context "when consuming" do
    context "from JSON" do
      ...
      it "consumes properties" do
        search_result = SearchResult.new.extend(subject).from_json(json)
